...
class KerasModel:
    ...
    def getFeatureVector(self,img1,log=True):
        ...
        a = image.load_img('a.jpg', target_size=(224, 224)) #loading image to the target size
        a_data = image.img_to_array(a) #converting the image to a numpy array
        a_data = np.expand_dims(a_data,axis=0)
        a_data = preprocess_input(a_data)
        a_feature = np.array(self.model.predict(a_data)).flatten() # fc[Fully Connected] layer vector: 2048
        if(log):
            print('.')
        
        os.remove('a.jpg')
        return a_feature # List of feature vectors of individual frames
